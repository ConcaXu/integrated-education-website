<template>
  <section class="activity-promo section"><div class="site-shell">
    ...
    <router-link class="button button-dark promo-all" to="/activities">{{ tx('View all activities', '查看全部活动') }}</router-link>
  </div></section>
</template>
...
const { lang } = useI18n(); const items = ref<ActivityItem[]>([]); const loading = ref(true)
const tx = (en: string, zh: string) => lang.value === 'en' ? en : zh
const formatDate = (value?: string) => value ? value.slice(0, 10).replace(/-/g, '.') : ''
onMounted(async () => { try { const response = await fetchActivityList(1, 3); if (response.code === 200) items.value = response.rows || [] } finally { loading.value = false } })
</script>
<style scoped>
